...
public class SobelEdgeDetectStage implements IStage
{
    ...
    public ShortImageBuffer flow(ShortImageBuffer before)
    {
        ...
        for(int y=1;y<before.getHeight()-1;y++)
        {
            ...
            {
                int g00 = before.get(y-1, x-1) & Constants.BYTE;
                int g01 = before.get(y-1, x) & Constants.BYTE;
                int g02 = before.get(y-1, x+1) & Constants.BYTE;
                int g10 = before.get(y, x-1) & Constants.BYTE;
                int g12 = before.get(y, x+1) & Constants.BYTE;
                int g20 = before.get(y+1, x-1) & Constants.BYTE;
                int g21 = before.get(y+1, x) & Constants.BYTE;
                int g22 = before.get(y+1, x+1) & Constants.BYTE;
